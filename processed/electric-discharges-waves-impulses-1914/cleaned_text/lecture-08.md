...
where <j) is the time angle, co the distance angle, u the exponential 
decrement, or the "power-dissipation constant," and ^o and eo the 
maximum current and voltage respectively. 

The power flow at any point of the circuit, that is, at any dis- 
tance angle co, and at any time t, that is, time angle 0, then is 

p = ei, 

= eo^e~2"* cos (0 =F co — 7) sin (0 =F co — 7), 
...
form circuit, the average flow of power, po, is zero, and no power 
...
power along the circuit, jpo, and a pulsation or surge, pi, of the same 
...
Such a flow of power must occur in a circuit containing sec- 
tions of different dissipation constants u. For instance, if in a 
circuit consisting of an unloaded transformer and a transmission 
line, as indicated in Fig. 40, at no load on the step-down trans- 
...
to introduce, when dealing with traveUng waves, the velocity unit 
as unit of length, that is, measure the length with the distance of 
propagation during unit time (3 X 10^° cm. with a straight con- 
...
wave length Xo and the period To by the same numerical values, 

Xo = Tq = -j., and makes the time angle </> and the distance angle co 
...
(h) The flow of power is decreasing in the direction of propaga- 
...
the energy dissipation by r and g, that is, by the decrement e~"^ 
Thus, in case (a) the flow of power along the circuit decreases at 


TRAVELING WAVES. 93 

the rate e"^', corresponding to the dissipation of the stored energy 
by €-"*, as indicated by A' in Fig. 42; while in the case (6) the 
...
line u should happen to be exactly the average, Uq, of the entire 
...
{h) If the flow of power decreases along the line, every line 
...
Since the current and voltage would decrease by the term e""^, 
...
passed. That is, as function of the distance, the factor e~ ^^ must 
...
e + ^^, and therefore decreases faster in the direction of progress 
by e~ ^^. That is, it scatters a part of its energy along its path 
of travel, and thus dies down more rapidly with the distance of 
travel. 

Thus, in a traveling wave of decreasing power flow, the time 
decrement is changed to e~*^"~^)^, and the distance decrement e+^^ 
added, and the equation of a traveling wave of decreasing power 
flow thus is 

2 = 2oe-("-s)<e-^^COS ((/)-6;-7)=2oe""^6+^('-^)cos ((/)-a;-7), ) 

e = e(^e-("-^'J<e-'''^C0S ((/)-co-7)=eoe~"^e+'^'~^) cos (0-a;-7); )^ ^ 

* Due to the use of the velocity unit of length X, distance and time are 
given the same units, to = Xoi and the time decrement, e+s<, and the distance 
decrement, e"-''^, give the same coefficient s in the exponent. Otherwise, the 
...
only in the form t — X and 0 — co respectively, and that thus for 
a constant value oi t — \ the decrement is e~^^^, that is, in the 
...
the intensity of the wave increases, by the same factor e+^^, or 
rather, the wave decreases along the line at a slower rate than 
corresponds to the power dissipation. 
The equations then become: 

Z = ^e-("+^)^e+'^^COS (0— CO — 7)=2oe-"'e-^('-^)cos (0— co — 7), 
e = eoe- ("+^)^ e+'^ COS (0 — co - 7) = eoe~^'e-^^^- ^) cos (0 — co — 7), 
...
and thus of oj would be reversed. 
...
I = 2oe- ("+«)' e ±«^ COS ((^Tco-y) =ioe- "^e-^^'^^^^cos ((/)Tco — 7), J 

e = eoe-^"+")*e^"^cos(0=Fco-7)=eoe-"'e-«^^'F=^) cos(0Tco-7), \ ^^^^ 

where the upper sign appUes to a wave traveling in the direction 
...
since, if — s > u, u -{- s would be negative, and €~("+®^^ would 
...
An oscillogram of a "grounded phase " on a lo4-mile three- 
...
in this case, w + s = 0, and the exponential function of time 
...
i = ioe+^'^ cos (0 — co — 71) — ^o'e~^^ (0 + co — 72), 
e = eoe+^^ cos (0 — co — 71) + ^o'e-^^ (0 + co — 72), 
...
the function of the time angle 0, to the standard form given in 
...
i = ioe- "^ e- « ^^ ±^^ cos (0 =F co - 7), 
e = eoe-''^e-«('±^) cos (0 =F co - 7), 
...
wave contains the factor e~^('^^), resulting from its progression 
...
Oscillation by Reflection of Impulse from Ends of Line. (The lowest curve 
gives a 60-cycle current as time measure.) 
...
Fig. 48 A gives the oscillation of the compound circuit consisting 
...
Fig. 48A. — CD10002. — Oscillogram of High-frequency Oscillation Preced- 
...
Fig. 48B gives the oscillation of the compound circuit consisting 
...
beginning of the oscillating circuit as eo. 
...
gap. These travehng waves of very high frequency are extremely 
...
relation -^ = \/ tt = ^q, discussed in Lecture VI. 
...
length. The inductance of this coil is calculated as approxi- 
mately 13 microhenrys. The line constants, from line to neutral, 

are L = 0.323 henry, C = 2.2 X IQ-' farad; hence Zq = y ^ = 

Vo.1465 X 10^ = 383 ohms. 
The sudden change of voltage at the line terminals, produced 

by closing the switch, is W- = 57,700 volts effective, or a 

V_3 

maximum of eo = 57,700 X V2 = 81,500 volts, and thus gives 

a maximum transient current in the impulse, of ^■o = — = 212 

2o 

amperes, io = 212 amperes maximum, traversing the inductance 
of 13 microhenrys, thus give the voltage, recorded by the spark 
gap, of ei = 35,000. If then / = frequency of impulse, it is 
...
one, which decreases very rapidly, e-(^ + ^^^, and thus preponder- 
ates in the beginning of the phenomenon, and one, which decreases 
slowly, e~^"~^^^ Hence it may be expressed in the form: 

Po = a^^-2{u + s)t ^+2s\ j^^^^-2{u-s)t^-2s\^ (20) 
...
thus of surge impedance Zq = sJ -^ = 590 ohms. 
...
Fig. 52. — CD11152. — Reproduction of Oscillogram of Propagation of Im- 
...
together with the primary current of the transformer, Zq. This 
oscillogram shows the reversals of current by reflection, and the 
formation of a stationary oscillation by the successive reflections 
