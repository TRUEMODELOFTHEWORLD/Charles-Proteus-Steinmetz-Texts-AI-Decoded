...
In ger^eral, in transmitting a given amount of power at a 
...
§ 260J COPPER EFFICIEXCY OF SYSTEMS. 381 
...
382 AL TEKXA TING-CURRENT PHENOMENA. [ § 260 

In the single-phase alternating-current circuit, if ^ = 
E.M.F., /= current, r= resistance per line, the total power 
is = ciy the loss of power 2/^^. 

Using, however, a three-wire system, the potential be- 
tween outside wires and neutral being given = ^, the 
potential between the outside wires is = 2 r, that is, the dis- 
...
the same cross-section with each of the outside wires, J of 
...
total power is 3 r i^ / V3 = c j\ V3. Hence if the same 
power has to be transmitted by the three-phase system as 
with the single-phase system, the three-phase line current 
must be t\ = // V3 ; hence if r^ = resistance of each of the 
three wires, the loss per wire is i^^ r^ = t^ rj S, and the 
total loss is /2ri, while in the single-phase system it is 
2 t\. Hence, to get the same loss, it must be : ri = r, that 
...
S 260J COPPER EFFICIENCY OF SYSTEMS. 383 
...
potential will be = r x V3, since the Y potential = Cy and 
the potential of the system is raised thereby from e to 
£ V3 ; that is, only J as much copper is required in the out- 
...
copper, or J = 33.3 per cent of the copper of the single- 
...
tral of full cross-section requires f'V = 31.25 per cent, the 
...
return and potential c per branch, denoting the current in 
the outside wires by i^, the current in the central wire is 
ij V2 ; and if the same current density is chosen for all 


384 AL TERNA TING-CURRENT PHENOMENA, [ § 260 

three wires, as the condition of maximum efficiency, and 
the resistance of each outside wire denoted by rg, the re- 
sistance of the central wire = r^fypi, and the loss of power 
per outside wire is i^ r^ , in the central wire 2 i^ r^ / v 2 
= i^ r^ V2 ; hence the total loss of power is 2 i^ r^ + i^ r^ 
V2 = i^ r^ (2 + V2). The power transmitted per branch 
is i^ Cy hence the total power 2 i^ e. To transmit the same 
power as by a single-phase system of power, ei, it must 

be i^ = t/2; hence the loss, — ^ — —, Since this 

loss shall be the same as the loss 2 i^ r in the single- 

v . •. «. u o (2 + V2) 8 r 
phase system, it must be 2 r =-^ — ■ -^ or i\ = : . 
...
E.M.Fs. ^ at 60® displacement, and three equal currents 
/g in the three lines of equal resistance rg, gives the out- 
put 2^*/3, that is, compared with the single-phase system, 
/g = //2. The loss in the three lines is 3 i^ ^3 = 3 ^^ 's- 
Hence, to give the same loss 2 /^ ;- as the single-phase sys- 
tem, it must be rg = 5 r, that is, each of the three wires 
must have J of the copper cross-section of the wire in the 
two-wire single-phase system ; or in other words, the in- 
verted three-phase system requires ^^ of the copper of the 
two-wire single-phase system. 


§ 260] COPPER EFFICIENCY OF SYSTEMS. 385 
...
386 AL TERNA TING-CURRENT PHENOMENA. [§261 

261. Comparison on the Basis of Equality of the Maximum 
Differetice of Potential in the System^ in Long- Distance 
Trafismissiofij Power Distribution^ etc. 
...
per branch will be ^/ V2, hence the current i^ = i I V2, if / 
...
Since in the single-phase system, the loss = 2/2r, it is : 
...
The central wires have to contain ^ V2 times as 
...
H 7 — V2 times as much copper as each of the single- 
...
relation (V3/^')2 : (2/^')2 = 3 : 4 ; that is, the three-phase 
...
I 262] COJ^PEK EFFICIENCY OF SYSTEMS, 389 
